...
class CO_Security_DB extends A_CO_DB {
    ...
    You give a security ID, and you will get all IDs that have that token in their list (or are of that ID).
    ...
    public function get_all_logins_with_access( $in_security_token  ///< An integer, with the requested security token.
        ...
        if ($this->access_object->god_mode() && $in_security_token) {
            ...
            if (isset($temp) && $temp && is_array($temp) && count($temp) ) {
                ...
                foreach ($temp as $result) {
                    $id = intval($result['ids']);
                    $ids = explode(',',$result['ids']);
                    
                    if ($in_security_token == $id) {
                        array_push($ret, $id);
                        continue;
                    } elseif (isset($ids) && is_array($ids) && count($ids)) {
                        foreach ($ids as $ind_id) {
                            $ind_id = intval($ind_id);
                            if ($in_security_token == $ind_id) {
                                array_push($ret, $id);
                                continue;
                            }
                        }
